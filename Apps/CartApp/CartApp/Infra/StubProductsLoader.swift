...
final class StubProductsLoader: ProductsLoader {
    
    func load() async throws -> [Product] {
        let response = HTTPURLResponse(url: Environment.productsURL, statusCode: 200, httpVersion: nil, headerFields: nil)!
        let products = try ProductsMapper.map(makeJSON(), from: response)
        return products
    }
    
    private func makeJSON() -> Data {
                """
                {
                    "products": [{
